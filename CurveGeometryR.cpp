...
int main (void)
{
  ...
  // the difference between these is 4 pi * SL
  double omegaSL = (omega-omega2)/(4.0*M_PI);
  // put omega in interval [0,4pi]
  while(omega>4*M_PI) omega -= 4*M_PI;
  while(omega<0) omega += 4*M_PI;
  // put omega2 in interval [0,4pi]
  while(omega2>4*M_PI) omega2 -= 4*M_PI;
  while(omega2<0) omega2 += 4*M_PI;

  // compute the regularised solid angle from the view point
  double omegaR = 2.0*M_PI*(1.0 + writhe) - SolidAngleCalcR(Curve,View);
  // put omega in interval [0,4pi]
  while(omegaR>4*M_PI) omegaR -= 4*M_PI;
  while(omegaR<0) omegaR += 4*M_PI;

  cout << "Wr = " << writhe << "  Tw = " << twist << "  SL = " << writhe+twist << endl;
  cout << "omega = " << omega << "  omega2 = " << omega2 << "  SL = " << omegaSL << endl;
...
void InitialiseCurve(knotcurve& Curve)
{
  ...
  */
  ...
  for (int s=0; s<NP; s++)
    {
      knotpoint Point;
      Point.xcoord = 1.0*cos(2.0*M_PI*s/NP);
      Point.ycoord = 1.0*sin(2.0*M_PI*s/NP);
  ...
  for(int s=0; s<NP; s++)    // new scheme -- central difference
    {
      ...
      // try this
      kappaNx -= (kappaNx*Curve.knotcurve[s].tx + kappaNy*Curve.knotcurve[s].ty + kappaNz*Curve.knotcurve[s].tz)*Curve.knotcurve[s].tx;
      kappaNy -= (kappaNx*Curve.knotcurve[s].tx + kappaNy*Curve.knotcurve[s].ty + kappaNz*Curve.knotcurve[s].tz)*Curve.knotcurve[s].ty;
      kappaNz -= (kappaNx*Curve.knotcurve[s].tx + kappaNy*Curve.knotcurve[s].ty + kappaNz*Curve.knotcurve[s].tz)*Curve.knotcurve[s].tz;
      ...
      Curve.knotcurve[s].curvature = sqrt(kappaNx*kappaNx + kappaNy*kappaNy + kappaNz*kappaNz);
    }
}
...
double Writhe(const knotcurve& Curve)
{
  ...
  for (int s=0; s<NP; s++) // running over the knot
      ...
      for (int t=s+1; t<NP; t++) // run over all points ahead of s
	{
	  ...
	  double dt = 0.5*(Curve.knotcurve[t].length+Curve.knotcurve[incp(t,-1,NP)].length);
  ...
  for (int s=0; s<NP; s++) // running over the knot
    {
      ...
      double ds0 = Curve.knotcurve[s].length;
      double dsp1 = Curve.knotcurve[incp(s,1,NP)].length;
      double dsm1 = Curve.knotcurve[incp(s,-1,NP)].length;
      double dsm2 = Curve.knotcurve[incp(s,-2,NP)].length;
      double ds;
      ...
      //      ds = 0.5*(Curve.knotcurve[s].length+Curve.knotcurve[incp(s,-1,NP)].length);
...
// regularised calculation of the integral for the solid angle
double SolidAngleCalcR(const knotcurve& Curve, const viewpoint& View)
{
  double Iplus = 0.0;
  int NP = Curve.knotcurve.size();

  // check proximity to tangent developable surface
  int M = 6; // even integer for defining a window around the cusp point
  ...
  for (int s=0; s<NP; s++)
    {
      ...
      double viewz = Curve.knotcurve[s].zcoord - View.zcoord;
      double dist = sqrt(viewx*viewx + viewy*viewy + viewz*viewz);      
      double ndotT = viewx*Curve.knotcurve[s].tx + viewy*Curve.knotcurve[s].ty + viewz*Curve.knotcurve[s].tz;
      // calculate dot product with the Frenet normal
      double ndotN = viewx*Curve.knotcurve[s].kappaNx + viewy*Curve.knotcurve[s].kappaNy + viewz*Curve.knotcurve[s].kappaNz;
      ndotN /= (dist*Curve.knotcurve[s].curvature); // shouldn't really be necessary
      // repeat for the next point along
      double viewx2 = Curve.knotcurve[incp(s,1,NP)].xcoord - View.xcoord;
      double viewy2 = Curve.knotcurve[incp(s,1,NP)].ycoord - View.ycoord;
      double viewz2 = Curve.knotcurve[incp(s,1,NP)].zcoord - View.zcoord;
      double dist2 = sqrt(viewx2*viewx2 + viewy2*viewy2 + viewz2*viewz2);      
      double ndotN2 = viewx*Curve.knotcurve[incp(s,1,NP)].kappaNx + viewy*Curve.knotcurve[incp(s,1,NP)].kappaNy + viewz*Curve.knotcurve[incp(s,1,NP)].kappaNz;
      ndotN2 /= (dist2*Curve.knotcurve[incp(s,1,NP)].curvature); // shouldn't really be necessary
      ...
      if ((ndotN*ndotN2<0) && (ndotT<0))
	{
	  ...
	  viewx = Curve.knotcurve[sp].xcoord - View.xcoord;
	  viewy = Curve.knotcurve[sp].ycoord - View.ycoord;
	  viewz = Curve.knotcurve[sp].zcoord - View.zcoord;
	  dist = sqrt(viewx*viewx + viewy*viewy + viewz*viewz);
	  // note the minus sign introduced here -- makes the quantity positive
	  ndotT = -(viewx*Curve.knotcurve[sp].tx + viewy*Curve.knotcurve[sp].ty + viewz*Curve.knotcurve[sp].tz)/dist;
	  // check if a threshold is exceeded
	  double threshold = Curve.knotcurve[sp].curvature*M*(Curve.length/NP)/(2.0*sqrt(2.0)*sqrt(1.0/ndotT - 1.0));
	  if (threshold > 1)
	    {
	      // note the minus sign introduced here -- makes the quantity positive
	      double ndotB = -(viewx*Curve.knotcurve[sp].kappaBx + viewy*Curve.knotcurve[sp].kappaBy + viewz*Curve.knotcurve[sp].kappaBz)/(dist*Curve.knotcurve[sp].curvature);
