#include "CurveGeometry_diagnostic.h"    
#include "Constants.h"    
#include <math.h>
#include <string.h>
...
int main (void)
{
    ...
    cout << "  writhe = " << Curve.Components[0].writhe <<  endl;
    cout << "  avglength = " << Curve.Components[0].length/Curve.Components[0].knotcurve.size() <<  endl;
...
double SolidAngleCalcR(const Link& Curve, const viewpoint& View, double& correctionapplied)
{
    ...
    for(int i=0; i<Curve.NumComponents; i++)
    {
            ...
            if (( (ndotN >0 && ndotN2 <0) || (ndotN<0 && ndotN2>0) ) && ndotT<0)
            {
                ...
                if (threshold > 0.5)
                {
                 ...
                 //       cout <<'\t' <<View.xcoord <<'\t' << View.ycoord <<'\t'  << View.zcoord<< '\n';
                    ...
                    double tau0 = Curve.Components[i].knotcurve[sp].torsion + lambda*(Curve.Components[i].knotcurve[incp(sp,1,NP)].torsion-Curve.Components[i].knotcurve[sp].torsion);
                 ...
                 //       correctedintegrand[incp(sp,q,NP)] = ( -k0*sintheta ) / (1-costheta+0.5*y1*y1*(k0*k0*costheta - k0*tau0*sintheta)) ;
                    }
                    // do the ones below 
                  ...
                  //      correctedintegrand[incp(sp,q,NP)] = ( -k0*sintheta ) / (1-costheta+0.5*y0*y0*(k0*k0*costheta - k0*tau0*sintheta)) ;
                    }
        ...
        {
            std::ostringstream oss;
            oss << "data/RegularisedPlusIntegrand" << View.xcoord << "_"  << View.ycoord <<"_" << View.zcoord << ".txt";
            string name = oss.str();
            ofstream Aout (name.c_str());
            ...
            Aout.close();
            std::ostringstream oss2;
            oss2 << "data/PlusIntegrand" << View.xcoord << "_"  << View.ycoord <<"_" << View.zcoord << ".txt";
            name = oss2.str();
            ofstream A2out (name.c_str());
